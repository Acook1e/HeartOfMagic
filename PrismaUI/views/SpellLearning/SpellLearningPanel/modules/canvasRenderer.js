...
var CanvasRenderer = {
    ...
    _computeSchoolAngles: function() {
        ...
        // Find root nodes for each school and compute their angles
        var rootAngles = {};
        schoolNames.forEach(function(name) {
            var school = self.schools[name];
            
            // If angles already set from data, use them
            if (school.spokeAngle !== undefined && school.startAngle !== undefined) {
                rootAngles[name] = school.spokeAngle;
                return;
            }
            
            // Find root node for this school
            var rootNode = self.nodes.find(function(n) { 
            ...
            if (!rootNode) {
                ...
                if (schoolNodes.length > 0) {
                    rootNode = schoolNodes.reduce(function(closest, n) {
                        var dist = Math.sqrt(n.x * n.x + n.y * n.y);
                        var closestDist = Math.sqrt(closest.x * closest.x + closest.y * closest.y);
            ...
            if (rootNode) {
                // Calculate angle of root node from center
                var angle = Math.atan2(rootNode.y, rootNode.x) * 180 / Math.PI;
                rootAngles[name] = angle;
            }
        });
        
        // Assign pie slices centered on root node angles
        schoolNames.forEach(function(name, i) {
            ...
            var rootAngle = rootAngles[name];
            if (rootAngle !== undefined) {
                // Center the pie slice on the root node's angle
                school.spokeAngle = rootAngle;
                school.startAngle = rootAngle - sliceAngle / 2;
                school.angleSpan = sliceAngle;
            } else {
                // Fallback to even distribution
                var startAngle = i * sliceAngle;
                school.startAngle = startAngle - 90;
                school.angleSpan = sliceAngle;
                school.spokeAngle = startAngle + sliceAngle / 2 - 90;
            }
        });
    },
    ...
    renderSchoolDividers: function(ctx) {
        ...
        // Draw dividers at each school's START angle (from actual data)
        for (var i = 0; i < schoolNames.length; i++) {
            var schoolName = schoolNames[i];
            var school = this.schools[schoolName];
            // Use actual startAngle from data, or fall back to computed
    ...
    renderEdges: function(ctx, viewLeft, viewRight, viewTop, viewBottom) {
        ...
        for (var i = 0; i < this.nodes.length; i++) {
            ...
            // Check if this is a root node (no prerequisites / tier 1)
            if (!node.isRoot && node.tier !== 1) continue;
        ...
        for (var i = 0; i < this.edges.length; i++) {
            ...
            if (isLearningEdge) {
                // Learning path - static line, pulses travel along it
                ctx.strokeStyle = learningPathColor;
    ...
    renderDebugGrid: function(ctx) {
        ...
        for (var schoolIdx = 0; schoolIdx < schoolNames.length; schoolIdx++) {
            var schoolName = schoolNames[schoolIdx];
            var school = this.schools[schoolName];
            
            // Use actual school angles from data
            var startAngle = school.startAngle !== undefined ? school.startAngle : (schoolIdx * (360 / schoolNames.length) - 90);
            var sliceAngle = school.angleSpan !== undefined ? school.angleSpan : (360 / schoolNames.length);
            var endAngle = startAngle + sliceAngle;
            
            // Use school color or fallback
            var color = TREE_CONFIG.getSchoolColor(schoolName) || defaultColors[schoolIdx % defaultColors.length];
            
            var startRad = startAngle * Math.PI / 180;
            var endRad = endAngle * Math.PI / 180;
            var outerRadius = baseRadius + maxTiers * tierSpacing;
            
            // Draw sector boundary line at START of this school's slice
            ctx.beginPath();
            ctx.moveTo(0, 0);
