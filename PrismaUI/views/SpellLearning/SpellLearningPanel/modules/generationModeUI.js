 ...
 * and per-school control panels with shape/density/flower settings.
 * 
 * Depends on:
...
var DEFAULT_AUTO_CONFIG_PROMPT = 
    ...
    '- flower_chance (0.05-0.30): Terminal cluster points\n' +
    '- flower_type: "hub"|"burst"|"cube"|"spiral"|"grid"\n\n' +
...
function createSchoolControlsHTML(schoolName) {
    ...
    html += '' +
        ...
        '</div>' +
        '<div class="school-control-row dev-only">' +
            '<span class="school-control-label">Flower %:</span>' +
            '<input type="range" class="school-control-slider" id="school-flower-' + safeId + '" ' +
                'min="5" max="30" value="15" oninput="updateSchoolSliderValue(this, 100)">' +
            '<span class="school-control-value" id="school-flower-val-' + safeId + '">15%</span>' +
        '</div>' +
        '<div class="school-control-row dev-only">' +
            '<span class="school-control-label">Flower Type:</span>' +
            '<select class="school-control-select" id="school-flower-type-' + safeId + '">' +
    ...
    html += '' +
        '<div class="school-control-actions">' +
            '<button class="btn btn-accent btn-sm" onclick="rerunSchool(\'' + schoolName + '\', \'complex\', true)" title="Generate with Python fuzzy NLP (requires Python)">Complex</button>' +
            '<button class="btn btn-secondary btn-sm" onclick="rerunSchool(\'' + schoolName + '\', \'simple\', true)" title="Generate with JavaScript only (no Python)">Simple</button>' +
        '</div>';
...
function updateSchoolSliderValue(slider, divisor, isInt) {
    ...
    if (valSpan) {
        ...
        } else if (divisor === 100 && slider.id.indexOf('flower') > -1) {
            valSpan.textContent = slider.value + '%';
        } else {
...
function getSchoolConfig(schoolName) {
    ...
    return {
        ...
        flower_chance: (document.getElementById('school-flower-' + safeId)?.value || 15) / 100,
        flower_type: document.getElementById('school-flower-type-' + safeId)?.value || 'burst',
...
function rerunSchool(schoolName, mode, useExistingSlice) {
    console.log('[GenerationModeUI] Rerun ' + schoolName + ' with mode: ' + mode + ', useExistingSlice: ' + useExistingSlice);
    var config = getSchoolConfig(schoolName);
    console.log('[GenerationModeUI] Config:', config);
    ...
    // Filter spells to this school
    var schoolSpells = state.lastSpellData.spells.filter(function(s) {
    ...
    // Config is already in the right format from getSchoolConfig
    var fullConfig = config;
    
    // Add flag to use existing slice
    fullConfig.useExistingSlice = useExistingSlice || false;
    
    if (mode === 'visualfirst' || mode === 'complex') {
        // Complex generation using Python fuzzy NLP
        setTimeout(function() {
            try {
                if (typeof generateVisualFirstTree === 'function') {
                    generateVisualFirstTree(schoolName, schoolSpells, fullConfig);
                    updateStatus('Complex: ' + schoolName + ' (' + schoolSpells.length + ' spells)');
                    setStatusIcon('OK');
                } else {
                    console.error('[GenerationModeUI] generateVisualFirstTree not available');
                    updateStatus('Tree generator not loaded');
                    setStatusIcon('X');
                }
            } catch (e) {
                console.error('[GenerationModeUI] Tree generation error:', e);
        ...
        // Simple generation using JavaScript only (grid positioning)
        setTimeout(function() {
            try {
                if (typeof generateSimpleSchoolTree === 'function') {
                    generateSimpleSchoolTree(schoolName, schoolSpells, fullConfig);
                    updateStatus('Simple: ' + schoolName + ' (' + schoolSpells.length + ' spells)');
                    setStatusIcon('OK');
                } else {
                    console.error('[GenerationModeUI] generateSimpleSchoolTree not available');
                    updateStatus('Simple generator not loaded');
                    setStatusIcon('X');
                }
            } catch (e) {
                console.error('[GenerationModeUI] Simple generation error:', e);
                updateStatus('Error: ' + e.message);
                setStatusIcon('X');
            }
        }, 50);
        ...
        // JavaScript generation for single school (legacy)
        setTimeout(function() {
            try {
                var treeData = buildSchoolTreeOnly(schoolName, schoolSpells, fullConfig);
                if (treeData) {
                    mergeSchoolTree(schoolName, treeData);
                    updateStatus('JS: Regenerated ' + schoolName + ' (' + schoolSpells.length + ' spells)');
                    setStatusIcon('OK');
                } else {
                    updateStatus('JS: Failed to generate ' + schoolName);
                    setStatusIcon('X');
                }
            } catch (e) {
                console.error('[GenerationModeUI] JS error:', e);
        ...
        if (typeof startProceduralPythonGenerate === 'function') {
            startProceduralPythonGenerate(schoolName, fullConfig);
        } else {
        ...
        // AI/LLM generation for single school
        if (typeof retrySpecificSchool === 'function') {
            retrySpecificSchool(schoolName);
        } else if (typeof window.retrySpecificSchool === 'function') {
            window.retrySpecificSchool(schoolName);
        } else {
            updateStatus('AI generation not available');
            setStatusIcon('X');
        }
    }
}
...
function buildSchoolTreeOnly(schoolName, spells, config) {
    ...
    if (typeof window.discoverThemes !== 'function' || typeof window.buildSchoolTree !== 'function') {
        ...
        console.log('[GenerationModeUI] buildSchoolTree:', typeof window.buildSchoolTree);
        return null;
    }
    ...
    try {
        ...
        var schoolTree = window.buildSchoolTree(schoolName, spells, themes);
