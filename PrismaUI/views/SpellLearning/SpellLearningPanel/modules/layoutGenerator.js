 ...
 * @param {Object} config - Additional config (flower_chance, etc.)
...
function generateFullGrid(sliceInfo, spellCount, shape, config) {
    ...
    var profile = SHAPE_PROFILES[shape] || SHAPE_PROFILES.organic;
    var shapeMask = SHAPE_MASKS[shape] || SHAPE_MASKS.radial;
    ...
    for (var tier = 0; tier < numTiers; tier++) {
        ...
        for (var i = 0; i < candidateCount; i++) {
            ...
            // Flowers only in outer tiers
            var flowerChance = config.flower_chance || 0.1;
            var isFlower = tier > 4 && rng() < flowerChance;
            ...
            positions.push({
                ...
                isRoot: false,
                isFlower: isFlower,
                flowerType: isFlower ? (config.flower_type || 'burst') : null,
                angleNorm: angleNorm,
                gridRadius: radius,
                gridAngle: baseAngle
            });
        ...
        for (var i = 0; i < positions.length; i++) {
            ...
            for (var j = i + 1; j < positions.length; j++) {
                ...
                if (dist < minDist && dist > 0.01) {
                    ...
                    if (!pj.isRoot && !pj.isFlower) {
                        pj.x += pushX;
                        pj.y += pushY;
                    }
                    if (!pi.isRoot && !pi.isFlower) {
                        pi.x -= pushX;
                        pi.y -= pushY;
                    }
                    moved = true;
...
/**
 * Select best positions to match spell count (called AFTER addFlowerChildren)
...
function selectPositions(positions, spellCount) {
    ...
    var flowers = selected.filter(function(p) { return p.isFlower && !p.isRoot; }).length;
    var flowerChildren = selected.filter(function(p) { return p.isFlowerChild; }).length;
    var regular = selected.length - roots - flowers - flowerChildren;
    
    console.log('[LayoutGenerator] Selected', selected.length, 'positions: roots=' + roots + 
                ', flowers=' + flowers + ', flowerChildren=' + flowerChildren + ', regular=' + regular);
...
function generateLayout(schoolsData, seed) {
    ...
    for (var schoolName in schoolsData) {
        ...
        // Add flower children if needed
        var rng = seededRandom(seed + hashString(schoolName));
        positions = addFlowerChildren(positions, config, rng, sliceInfo);
...
/**
 * Add flower children around flower nodes.
 */
function addFlowerChildren(positions, config, rng, sliceInfo) {
    var flowerNodes = positions.filter(function(p) { return p.isFlower && !p._flowerProcessed; });
    var flowerType = config.flower_type || 'burst';
    
    // Sector boundary info for clamping - MORE CONSERVATIVE padding to prevent leaking
    var spokeAngle = sliceInfo ? sliceInfo.spokeAngle : 0;
    var sectorPadding = 8;  // Degrees - more padding than before to keep flowers/children inside
    var sectorHalf = sliceInfo ? (sliceInfo.sectorAngle / 2 - sectorPadding) : 180;
    ...
    function clampToSector(x, y) {
        if (!sliceInfo) return { x: x, y: y, angle: Math.atan2(y, x) * 180 / Math.PI };
        
        var currentAngle = Math.atan2(y, x) * 180 / Math.PI;
        var radius = Math.sqrt(x * x + y * y);
        var diff = angleDiff(currentAngle, spokeAngle);
        
        if (Math.abs(diff) > sectorHalf) {
            var clampedAngle = spokeAngle + (diff > 0 ? sectorHalf : -sectorHalf);
            return {
                x: Math.cos(clampedAngle * Math.PI / 180) * radius,
                y: Math.sin(clampedAngle * Math.PI / 180) * radius,
                angle: clampedAngle
            };
        }
        return { x: x, y: y, angle: currentAngle };
    }
    
    // FLOWER_PATTERNS - radius scaled to nodeSize
    // Increased radius to spread children further apart and prevent overlap
    var ns = LAYOUT_CONFIG.nodeSize;
    var FLOWER_PATTERNS = {
        burst: { points: 4, radius: Math.round(ns * 1.8), angleOffset: 45 },   // 4 children, spread wider
        spiral: { points: 5, radius: Math.round(ns * 2.0), spiralFactor: 0.4 }, // 5 children, larger spiral
        cube: { points: 4, radius: Math.round(ns * 1.6), angleOffset: 45 },     // 4 children in X shape
        hub: { points: 4, radius: Math.round(ns * 1.7), angleOffset: 0 }        // 4 children
    };
    
    var pattern = FLOWER_PATTERNS[flowerType] || FLOWER_PATTERNS.burst;
    var minSpacing = LAYOUT_CONFIG.nodeSize * 2.0;  // Increased spacing
    // Reserve more space around flowers (1.5x the child radius)
    var flowerInfluenceRadius = pattern.radius * 1.5 + LAYOUT_CONFIG.nodeSize;
    
    flowerNodes.forEach(function(flower) {
        flower._flowerProcessed = true;
        
        // Remove nodes within flower's zone and mark for relocation
        var nodesToRelocate = [];
        for (var i = positions.length - 1; i >= 0; i--) {
            var pos = positions[i];
            if (pos === flower || pos.isFlowerChild || pos.isRoot) continue;
            
            var dx = pos.x - flower.x;
            var dy = pos.y - flower.y;
            var dist = Math.sqrt(dx * dx + dy * dy);
            
            if (dist < flowerInfluenceRadius) {
                nodesToRelocate.push(positions.splice(i, 1)[0]);
            }
        }
        
        // Add flower children in a circle AROUND the flower (not extending outward)
        var numChildren = pattern.points;
        var childRadius = pattern.radius;
        
        for (var i = 0; i < numChildren; i++) {
            // Local angle - distribute evenly around flower center, NOT based on flower's wheel position
            var localAngle = (360 / numChildren) * i + (pattern.angleOffset || 0);
            var actualRadius = childRadius;
            if (pattern.spiralFactor) {
                actualRadius = childRadius * (0.7 + i * pattern.spiralFactor / numChildren);
            }
            
            // Position relative to flower center using LOCAL angle
            var childX = flower.x + Math.cos(localAngle * Math.PI / 180) * actualRadius;
            var childY = flower.y + Math.sin(localAngle * Math.PI / 180) * actualRadius;
            
            // Clamp to sector boundaries
            var clamped = clampToSector(childX, childY);
            childX = clamped.x;
            childY = clamped.y;
            
            // Calculate child's wheel position (for rendering)
            var childWheelRadius = Math.sqrt(childX * childX + childY * childY);
            var childWheelAngle = clamped.angle || Math.atan2(childY, childX) * 180 / Math.PI;
            
            positions.push({
                tier: flower.tier,  // Same tier as flower (not +0.5)
                radius: childWheelRadius,
                angle: childWheelAngle,
                x: childX,
                y: childY,
                isRoot: false,
                isFlower: false,
        ...
        // Relocate displaced nodes (passing sliceInfo for boundary checking)
        nodesToRelocate.forEach(function(displaced) {
            var relocated = findSafePosition(positions, displaced, flower, rng, flowerInfluenceRadius, sliceInfo);
            if (relocated) {
                positions.push(relocated);
            }
        });
    });
...
/**
 * Find a safe position for a displaced node (respects sector boundaries).
 */
function findSafePosition(positions, original, avoidFlower, rng, flowerRadius, sliceInfo) {
    var minSpacing = LAYOUT_CONFIG.nodeSize * 1.5;  // Web ratio
    var maxAttempts = 20;
    
    // Sector info for validation
    var spokeAngle = sliceInfo ? sliceInfo.spokeAngle : 0;
    var sectorPadding = 8;
    var sectorHalf = sliceInfo ? (sliceInfo.sectorAngle / 2 - sectorPadding) : 180;
    ...
    function isWithinSector(x, y) {
        if (!sliceInfo) return true;
        var angle = Math.atan2(y, x) * 180 / Math.PI;
        return Math.abs(angleDiff(angle, spokeAngle)) <= sectorHalf;
    }
    
    for (var attempt = 0; attempt < maxAttempts; attempt++) {
        var pushAngle = Math.atan2(original.y - avoidFlower.y, original.x - avoidFlower.x);
        var pushDist = LAYOUT_CONFIG.nodeSize * (1.5 + attempt * 0.3);  // Scaled push
        
        // Bias push toward the center of the sector (radial outward)
        var centerAngle = Math.atan2(avoidFlower.y, avoidFlower.x);
        var radialBias = 0.6 + rng() * 0.4;  // 60-100% radial bias
        pushAngle = pushAngle * (1 - radialBias) + centerAngle * radialBias;
        pushAngle += (rng() - 0.5) * 0.4;  // Reduced randomness
        
        var newX = avoidFlower.x + Math.cos(pushAngle) * (flowerRadius + pushDist);
        var newY = avoidFlower.y + Math.sin(pushAngle) * (flowerRadius + pushDist);
        
        // Check if within sector FIRST
        if (!isWithinSector(newX, newY)) {
            continue;  // Skip positions outside sector
        }
        ...
        if (clear) {
            return {
                ...
                angle: Math.atan2(newY, newX) * 180 / Math.PI,
                x: newX,
                y: newY,
                isRoot: false,
                isFlower: false,
    ...
    return null;  // Give up - node doesn't fit
}
...
window.LayoutGenerator = {
    ...
    addFlowerChildren: addFlowerChildren,
