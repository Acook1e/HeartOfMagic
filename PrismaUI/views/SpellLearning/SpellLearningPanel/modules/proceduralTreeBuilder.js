...
function buildSchoolTree(schoolName, spells, themes) {
...
function buildProceduralTrees(spells) {
    ...
    for (var schoolName in schoolSpells) {
        ...
        var schoolTree = buildSchoolTree(schoolName, schoolSpells[schoolName], themes);
        if (schoolTree) {
...
function generateSimpleSchoolTree(schoolName, spells, config) {
    ...
    var schoolTree = buildSchoolTree(schoolName, spells, themes);
...
function startProceduralPythonGenerate(schoolFilter, schoolConfig) {
    ...
    var spellsToProcess = state.lastSpellData.spells;
    ...
    var llmEnabled = config.llm_auto_configure.enabled || config.llm_groups.enabled;
...
function applySchoolConfigsToUI(schoolConfigs) {
    for (var schoolName in schoolConfigs) {
        ...
        // Update flower chance slider
        var flowerSlider = document.getElementById('school-flower-' + safeId);
        var flowerVal = document.getElementById('school-flower-val-' + safeId);
        if (flowerSlider && cfg.flower_chance !== undefined) {
            flowerSlider.value = Math.round(cfg.flower_chance * 100);
            if (flowerVal) flowerVal.textContent = Math.round(cfg.flower_chance * 100) + '%';
        }
        
        // Update flower type dropdown
        var flowerTypeSelect = document.getElementById('school-flower-type-' + safeId);
        if (flowerTypeSelect && cfg.flower_type) {
            flowerTypeSelect.value = cfg.flower_type;
        }
        
        // Update burst min slider
        var burstMinSlider = document.getElementById('school-burst-min-' + safeId);
        var burstMinVal = document.getElementById('school-burst-min-val-' + safeId);
        if (burstMinSlider && cfg.flower_burst_min !== undefined) {
            burstMinSlider.value = cfg.flower_burst_min;
            if (burstMinVal) burstMinVal.textContent = cfg.flower_burst_min;
        }
        
        // Update burst max slider
        var burstMaxSlider = document.getElementById('school-burst-max-' + safeId);
        var burstMaxVal = document.getElementById('school-burst-max-val-' + safeId);
        if (burstMaxSlider && cfg.flower_burst_max !== undefined) {
            burstMaxSlider.value = cfg.flower_burst_max;
            if (burstMaxVal) burstMaxVal.textContent = cfg.flower_burst_max;
        }
...
var FALLBACK_AUTO_CONFIG_PROMPT = 
    ...
    '{"SchoolName": {"shape": "...", "growth_behavior": "...", "branching_mode": "fuzzy_groups", "slice_weight": 1.0, "density": 0.6, "flower_chance": 0.15, "flower_type": "burst", "branch_chance": 0.25, "branch_energy_gain": 0.12, "branch_energy_threshold": 1.8, "branch_subdivide_pool": true, "alt_path_min_distance": 4, "alt_path_max_distance": 4, "alt_path_probability": 0.3, "alt_path_max_per_node": 2, "reasoning": "..."}, ...}';
    ...
    if (schoolConfigs && Object.keys(schoolConfigs).length > 0) {
        for (var sc in schoolConfigs) {
            ...
            console.log('  ' + sc + ': shape=' + cfg.shape + ', density=' + cfg.density + 
                        ', flower=' + cfg.flower_chance + ', source=' + (cfg.source || '?'));
        }
    } else {
    ...
    setTimeout(function() {
        try {
            ...
            for (var schoolName in spellsBySchool) {
                ...
                config.flower_chance = config.flower_chance || 0.1;
                config.flower_type = config.flower_type || 'burst';
            ...
            // Call visual-first builder with fuzzy data for edge building
            if (typeof generateAllVisualFirstTrees === 'function') {
                console.log('[VisualFirst] Calling generateAllVisualFirstTrees with fuzzy data');
                var treeData = generateAllVisualFirstTrees(state.lastSpellData.spells, finalConfigs, fuzzy);
                
                // DIAGNOSTIC: Check tree data before loadTreeData
                console.log('[VisualFirst] === DIAGNOSTIC: Tree data before loadTreeData ===');
                for (var scName in treeData.schools) {
                    var school = treeData.schools[scName];
                    var firstNode = school.nodes[0];
                    console.log('[VisualFirst] ' + scName + ' (' + school.nodes.length + ' nodes):');
                    if (firstNode) {
                        console.log('[VisualFirst]   First node: x=' + firstNode.x + ', y=' + firstNode.y + 
                                    ', _fromVisualFirst=' + firstNode._fromVisualFirst);
                    }
                }
                ...
                var statusMsg = 'Complex Build: ' + schoolCount + ' schools, ' + nodeCount + ' spells';
                if (llmCount > 0) {
                    statusMsg += ' (LLM: ' + llmCount + ')';
                }
                updateStatus(statusMsg);
                setStatusIcon('OK');
                ...
                console.error('[VisualFirst] generateAllVisualFirstTrees not available');
                updateStatus('Visual-first builder not loaded');
                setStatusIcon('X');
            }
