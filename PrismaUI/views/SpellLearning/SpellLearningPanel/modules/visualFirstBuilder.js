 ...
 * 2. Flower application
 * 3. Spell assignment to positions
 * 4. Edge building (proximity-based)
 * 5. Output formatting
 * 
 * Depends on: layoutGenerator.js, flowerFormations.js, state.js
 */
 ...
 * 
 * @param {Array} positions - All positions (including flower children)
...
function assignSpellsToPositions(positions, spells, seed) {
    ...
    var regularPositions = positions.filter(function(p) { return !p.isFlowerChild && !p.isRoot; });
    var flowerChildPositions = positions.filter(function(p) { return p.isFlowerChild; });
    ...
    // Step 4: Assign remaining to flower children
    for (var i = 0; i < flowerChildPositions.length && allSpellsOrdered.length > 0; i++) {
        flowerChildPositions[i].spell = allSpellsOrdered.shift();
        assignedCount++;
    }
 ...
 * 4. Flower children connect only to their parent flower
 * 5. Fuzzy relationships influence prerequisite selection (prefer related spells)
 * 
...
function buildEdges(positions, config, seed, fuzzyData) {
    ...
    // Group by effective tier (considering flower children at tier+0.5)
    var byTier = {};
    nodesWithSpells.forEach(function(p) {
    ...
    for (var tIdx = 1; tIdx < tierNums.length; tIdx++) {
        ...
        currentTier.forEach(function(pos) {
            ...
            // Flower children ONLY connect to their parent flower
            if (pos.isFlowerChild) {
                if (pos.parentFlower && pos.parentFlower.spell) {
                    edges.push({
                        from: pos.parentFlower.spell.formId,
                        to: myFormId,
                        type: 'flower'
                    });
            ...
            // Score candidates by: fuzzy relationship > similarity > distance
            var scoredCandidates = connectedPrevious.map(function(c) {
                ...
                // Scoring: fuzzy related = +1000, similarity adds score, distance subtracts
                var score = 0;
                if (isRelated) score += 1000;
            ...
            if (scoredCandidates.length > 0) {
                ...
                edges.push({
                    ...
                    to: myFormId,
                    type: best.isRelated ? 'fuzzy_primary' : 'primary'
                });
                connectedToRoot[myFormId] = true;
                totalEdges++;
            ...
            if (scoredCandidates.length > 1 && rng() < convergence) {
                ...
                var candidates = scoredCandidates.slice(1).filter(function(c) {
                    // Must be sufficiently far from first choice (different branch)
                    return distance(c.pos, scoredCandidates[0].pos) > 40;
                });
                ...
                if (candidates.length > 0) {
                    ...
                    edges.push({
                        from: scoredCandidates[1].pos.spell.formId,
                        to: myFormId,
                        type: 'convergence'
                    });
                    totalEdges++;
...
function growOrganicTree(spells, sliceInfo, config, seed) {
    ...
    while (spellQueue.length > 0) {
        ...
        var params = typeof getActiveParameters === 'function' 
    ...
    var prereqResult = addPrerequisiteLinks(positions, edges, spellToGroup, rng);
...
function findBehaviorParent(placedNodes, hubNodes, group, params, rng, childSpell) {
    ...
    candidates.sort(function(a, b) {
        ...
        // child count * 3 + level diff * 2 + level penalty + root penalty + group bonus + thematic bonus
        var aScore = aChildren * 3 + aLevelDiff * 2 + aLevelPenalty + aRootPenalty + aGroupBonus + aThematicBonus;
        var bScore = bChildren * 3 + bLevelDiff * 2 + bLevelPenalty + bRootPenalty + bGroupBonus + bThematicBonus;
 ...
 */
function calculatePrereqScore(candidate, target, spellToGroup) {
    var score = 0;
 ...
 */
function addPrerequisiteLinks(positions, edges, spellToGroup, rng) {
    var prereqsAdded = 0;
    ...
    positions.forEach(function(targetNode) {
        ...
        positions.forEach(function(candidateNode) {
            ...
            var score = calculatePrereqScore(candidateNode, targetNode, spellToGroup);
            if (score > 0) {
 ...
 */
function calculateAlternatePathScore(nodeA, nodeB, pathDistance, spellToGroup) {
    var score = 0;
    var nodeSize = GRID_CONFIG.nodeSize;
 ...
 * @param {Object} config - LLM config {shape, density, convergence, flower_chance, flower_type, slice_weight, useExistingSlice}
...
function generateVisualFirstTree(schoolName, spells, config) {
    ...
    if (branchingMode === 'fuzzy_groups') {
        ...
        // Step 1: Generate layout (grid + selection) - NO FLOWERS
        var layout = generateLayout(schoolsData, seed);
        var schoolLayout = layout.schools[schoolName];
        
        // Step 2: Assign spells to positions (NO flowers - exact grid only)
 ...
 */
function generateAllVisualFirstTrees(allSpells, schoolConfigs, fuzzyData) {
    ...
    for (var schoolName in schoolsData) {
        ...
        if (branchingMode === 'fuzzy_groups') {
            ...
            // PROXIMITY MODE: Grid-based layout (NO FLOWERS - exact grid only)
            ...
            // Select best positions to match spell count (NO flower children)
            positions = selectPositions(positions, spells.length);
            
            schoolLayout = { positions: positions, sliceInfo: sliceInfo };
            
            console.log('[VisualFirstBuilder] Grid: ' + positions.length + ' positions (exact grid, no flowers)');
