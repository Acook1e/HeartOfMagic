 ...
 * 
 * Depends on: TREE_CONFIG, settings, state, SpellCache, TreeParser, GROWTH_DSL
 */
...
var WheelRenderer = {
    ...
    // Visual shape modifiers - translate backend shapes to DRAMATIC visual effects
    ...
    renderDebugGrid: function() {
        ...
        // Get school slices (5 schools, equal slices)
        var numSchools = 5;
        ...
        for (var schoolIdx = 0; schoolIdx < numSchools; schoolIdx++) {
            var startAngle = schoolIdx * sliceAngle - 90;  // -90 to start from top
    ...
    setSchoolConfigs: function(configs) {
        ...
        for (var school in this.schoolConfigs) {
            ...
            console.log('[WheelRenderer]   ' + school + ': shape=' + cfg.shape + 
                ...
                ', flower=' + ((cfg.flower_chance || 0) * 100).toFixed(0) + '%');
        }
    },
    ...
    getSchoolVisualModifier: function(schoolName) {
        ...
        var modifier = this.shapeVisualModifiers[shape] || this.shapeVisualModifiers.organic;
        
        // Get config values with defaults
        var density = cfg ? (cfg.density || 0.6) : 0.6;
        var symmetry = cfg ? (cfg.symmetry || 0.3) : 0.3;
        var flowerChance = cfg ? (cfg.flower_chance || 0.15) : 0.15;
    ...
    layoutRadial: function() {
        ...
        // Check if nodes have pre-computed positions from visual-first builder
        var nodesWithPos = this.nodes.filter(function(n) {
            return n._visualFirstX !== undefined || (n.x !== undefined && n.y !== undefined && n._fromVisualFirst);
        });
        ...
        if (hasPrecomputedPositions) {
            console.log('[WheelRenderer] Using pre-computed visual-first positions');
    ...
    layoutSectorsOnly: function() {
        ...
        if (hasSliceInfo) {
            ...
            // FALLBACK: Calculate weighted sectors (may not match exactly)
            console.log('[WheelRenderer] No sliceInfo, calculating weighted sectors');
            
            var totalWeight = 0;
            var schoolWeights = {};
            visibleSchools.forEach(function(name) {
                var school = self.schools[name];
                var nodeCount = self.nodes.filter(function(n) { return n.school === name; }).length;
                var weight = school.config?.slice_weight || 1.0;
                schoolWeights[name] = nodeCount * weight;
                totalWeight += schoolWeights[name];
            });
            
            var totalPadding = numSchools * cfg.schoolPadding;
            var availableAngle = 360 - totalPadding;
            var currentAngle = -90;  // Start at top (matching layoutGenerator)
            
            visibleSchools.forEach(function(schoolName) {
                var school = self.schools[schoolName];
                
                var sectorAngle = (schoolWeights[schoolName] / totalWeight) * availableAngle;
                
                school.startAngle = currentAngle;
                school.endAngle = currentAngle + sectorAngle;
                school.spokeAngle = currentAngle + sectorAngle / 2;
                ...
                currentAngle += sectorAngle + cfg.schoolPadding;
            });
        }
    },
        ...
        for (var d = 0; d <= school.maxDepth; d++) {
            ...
            if (d === 0) {
                tier.forEach(function(node, j) {
                    var angleOffset = 0;
                    if (tier.length > 1) {
                        var rootSpread = Math.min(effectiveSectorAngle * 0.3, 30);
                ...
                if (fillTriangle) {
                    // MOUNTAIN/TRIANGLE FILL: spread DECREASES with depth (wide base, narrow peak)
                    // At depth 0: use full sector, at max depth: use taperAmount of sector
                ...
                tier.forEach(function(node, j) {
                    ...
                    if (tier.length > 1) {
                        ...
                        // Single node in tier for triangle fill - add some random offset within allowed spread
                        angleOffset = (seededRandom() - 0.5) * spreadAngle * 0.5;
                    }
                    
                    // Apply jitter based on visual modifier (less jitter for symmetric shapes)
                    ...
                    // Apply group-specific modifiers (if node belongs to an LLM group)
                    ...
                    // Enforce strict pie slice boundaries if enabled
    ...
    createFullNode: function(node) {
        ...
        // Check for LLM group color override and flower status
        var groupColor = this.getNodeGroupColor(node);
        var color = groupColor || TREE_CONFIG.getSchoolColor(node.school);
        var isFlower = node.is_flower || node.isFlower || false;
        
        // FLOWER NODES: Just add class for styling, no glow
        if (isFlower) {
        ...
        // Flower nodes get extra border highlight
        if (isFlower && !groupColor && node.state !== 'unlocked') {
            bgShape.setAttribute('stroke', color);
            bgShape.setAttribute('stroke-width', '2');
            bgShape.setAttribute('stroke-opacity', '0.7');
        }
    ...
    getNodeDisplayName: function(node, nodeWidth) {
        ...
        // AVAILABLE (learnable) nodes - always reveal name since player can learn them
        ...
        // Other states: check progressive reveal threshold
    ...
    showTooltip: function(node, event) {
        ...
        var showName = showFullInfo || isLearnable || progressPercent >= settings.revealName || isRootWithReveal;
