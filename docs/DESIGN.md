...
### System 4: Progression Controller (Future)
- Tracks which spells player has unlocked
- Enforces prerequisite requirements
- Grants spells when unlocked
...
- [ ] Create basic HTML/CSS/JS view for SpellLearningPanel
- [ ] Register F9 hotkey to toggle panel visibility
- [ ] Panel shows: title, status text, scan button, editable text area

#### 1.2 SKSE Plugin Foundation
- [ ] Create C++ plugin with CMake build
- [ ] Register with PrismaUI for view management
- [ ] Hotkey handler (F9 = scancode 67)
- [ ] Basic logging

#### 1.3 Spell Scanning
- [ ] Iterate all SpellItem forms from data handler
- [ ] Extract relevant spell data:
  ...
  - Keywords
- [ ] Filter to player-learnable spells (exclude abilities, powers, lesser powers)

#### 1.4 LLM Prompt Generation
- [ ] Generate structured JSON of all spells
...
- [ ] Save prompt + spell data to file
- [ ] Display in UI text area (editable)

#### 1.5 Testing Tier 1
- [ ] Launch game, press F9, verify panel appears
- [ ] Click scan button, verify spells are scanned (check logs)
- [ ] Verify output file created with spell data + prompt
- [ ] Verify text area shows the content

---

### Tier 2: LLM Integration & Tree Building
**Goal:** Process LLM output and build the spell tree data structure
