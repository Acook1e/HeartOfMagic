...
constexpr const char *DEST_PLUGIN_NAMES[] = {
    ...
    "ISL-DESTified.esl"};
constexpr size_t DEST_PLUGIN_COUNT =
    sizeof(DEST_PLUGIN_NAMES) / sizeof(DEST_PLUGIN_NAMES[0]);
...
void Initialize() {
  ...
  if (g_destInstalled) {
    ...
    logger::info("DESTIntegration: Don't Eat Spell Tomes not found, "
...
bool IsActive() { return g_active && g_destInstalled && g_config.enabled; }
...
void SetConfig(const DESTConfig &config) {
  ...
  if (g_destInstalled) {
    ...
    logger::info("DESTIntegration: Config updated - enabled: {}",
                 g_config.enabled);
  }
}
...
bool OnSpellTomeRead(RE::TESObjectBOOK *book, RE::SpellItem *spell,
  ...
  if (!book || !spell) {
    logger::warn(
        "DESTIntegration: OnSpellTomeRead called with null book or spell");
    return false;
  }

  logger::info("DESTIntegration: Spell tome read - {} ({})", book->GetName(),
               spell->GetName());
  ...
  if (!pm->IsSpellAvailableToLearn(formIdStr)) {
    RE::SendHUDMessage::ShowHUDMessage(
        "You lack the knowledge to grasp this magic.");
    logger::info("DESTIntegration: Spell {} not available to learn "
  ...
  if (requiredXP <= 0) {
    ...
    logger::warn(
        "DESTIntegration: No required XP found for {}, using default {}",
        formIdStr, requiredXP);
  }
  ...
  logger::info(
      "DESTIntegration: Granting {:.0f} XP to {} ({}% of {:.0f} required)",
  ...
  snprintf(notification, sizeof(notification), "You begin to grasp %s...",
           spell->GetName());
  ...
  logger::info("DESTIntegration: Successfully processed tome for {} - player "
               "now has early access",
               spell->GetName());
...
bool IsIntegrationActive(RE::StaticFunctionTag *) { return IsActive(); }
} // namespace Papyrus

bool RegisterPapyrusFunctions(RE::BSScript::IVirtualMachine *vm) {
  if (!vm) {
    logger::error(
        "DESTIntegration: Failed to register Papyrus functions - VM is null");
    return false;
  }

  // Register under new script name SpellLearning_DEST
  vm->RegisterFunction("OnTomeRead", "SpellLearning_DEST", Papyrus::OnTomeRead);
  vm->RegisterFunction("IsIntegrationActive", "SpellLearning_DEST",
                       Papyrus::IsIntegrationActive);
  ...
  vm->RegisterFunction("IsIntegrationActive", "SpellLearning_ISL",
                       Papyrus::IsIntegrationActive);
