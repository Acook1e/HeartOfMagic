...
void SetupLog()
{
  ...
  *path /= std::string(SKSE::GetPluginName()) + ".log";
...
public:
  ...
  RE::BSEventNotifyControl ProcessEvent(RE::InputEvent* const* a_event, RE::BSTEventSource<RE::InputEvent*>*) override
  {
    ...
    for (auto* event = *a_event; event; event = event->next) {
      ...
      // PERFORMANCE: Check key code before IsDown (idCode is a simple member
      // access)
      ...
      // Don't break - continue processing in case there are multiple hotkey
      // events
    }
...
void OnDataLoaded()
{
  ...
  // Note: ISLIntegration removed - SpellTomeHook now handles spell tome
  // interception directly in C++

  // Register and initialize XP sources (spell casting only - tomes handled by
  // SpellTomeHook)
  ...
  logger::info("XP sources registered: {} total, {} active", registry.GetAll().size(), registry.GetActive().size());
}
...
void OnPostLoadGame()
{
  ...
  // Fix input/focus state that may be left bad by other mods or previous
  // session
  if (UIManager::GetSingleton()->IsInitialized()) {
    UIManager::GetSingleton()->EnsureFocusReleased();
  }
...
void MessageHandler(SKSE::MessagingInterface::Message* a_msg)
{
  ...
  case SKSE::MessagingInterface::kPostLoad:
    ...
    if (SpellTomeHook::Install()) {
      ...
      logger::error("SpellTomeHook failed to install - spell tomes will use "
                    "vanilla behavior");
    }
    break;
...
SKSEPluginLoad(const SKSE::LoadInterface* skse)
{
  ...
  if (papyrus) {
    ...
    logger::info("Papyrus API registered - other mods can call "
                 "SpellLearning.OpenMenu() etc.");
  } else {
