...
struct AdjustForPerksHook {
  ...
  static void thunk(RE::ActiveEffect *a_effect, RE::Actor *a_caster,
    ...
    // PERFORMANCE: Use cached player pointer instead of GetSingleton() every
    // call
    if (!s_cachedPlayer) {
    ...
    SpellEffectivenessHook::GetSingleton()->ApplyEffectivenessScalingFast(
        a_effect);
  }
  ...
  static void Install() {
    // AdjustForPerks is at virtual index 0 in ActiveEffect
    REL::Relocation<std::uintptr_t> vtbl{RE::VTABLE_ActiveEffect[0]};
    func = vtbl.write_vfunc(0x0, thunk); // Index 0 for AdjustForPerks

    logger::info("SpellEffectivenessHook: AdjustForPerks hook installed at "
                 "vtable index 0");
  }
};

void SpellEffectivenessHook::Install() {
  logger::info("SpellEffectivenessHook: Installing hooks...");

  // Hook for magnitude scaling (effectiveness) - scales early-learned spell
  // damage
  AdjustForPerksHook::Install();

  logger::info("SpellEffectivenessHook: Installation complete");
}

// =============================================================================
// SPELL NAME DISPLAY HOOK
// =============================================================================
// Hooks SpellItem's GetFullName to show "(Learning - X%)" for early-learned
// spells
...
struct SpellNameHook {
  ...
  static const char *thunk(RE::TESFullName *a_fullName) {
    ...
    std::call_once(s_firstLogFlag, [&]() {
      logger::info(
          "SpellNameHook: Hook active - first spell queried: {} ({:08X})",
    ...
    if (!g_modifiedName.empty()) {
      ...
      logger::trace("SpellNameHook: Returning modified name for {:08X}",
                    spellId);
  ...
  static void Install() {
    ...
    logger::info("SpellEffectivenessHook: SpellItem TESFullName::GetFullName "
                 "hook installed (vtable[1], index 5)");
  }
};
...
struct MagicMenuUIHook {
  ...
  static inline std::unordered_map<RE::MagicMenu *,
                                   std::chrono::steady_clock::time_point>
      s_lastUpdateTime;
  static inline std::mutex s_updateMutex;
  static constexpr auto UPDATE_INTERVAL_MS =
  ...
  static void UpdateSpellNamesInMenu(RE::MagicMenu *a_menu) {
    ...
    if (firstAttempt) {
      logger::info("MagicMenuUIHook: First update attempt - checking for "
                   "early-learned spells");

      // Log what spells are tracked as early-learned
      auto earlySpells = hook->GetEarlyLearnedSpells();
      logger::info("MagicMenuUIHook: {} early-learned spells tracked",
                   earlySpells.size());
      for (RE::FormID spellId : earlySpells) {
        auto *spell = RE::TESForm::LookupByID<RE::SpellItem>(spellId);
        logger::info("  - {:08X} '{}'", spellId,
                     spell ? spell->GetName() : "UNKNOWN");
      }
      firstAttempt = false;
    }
  ...
  static void UpdateSpellNameInGFx(RE::GFxValue &root, RE::FormID spellId,
  ...
  static bool TryUpdateInItemList(RE::GFxValue &parent, RE::FormID spellId,
                                  const std::string &modifiedName,
                                  const char *originalName) {
    ...
    for (std::uint32_t i = 0; i < arraySize; ++i) {
      ...
      if (entry.GetMember("formId", &formIdValue) && formIdValue.IsNumber()) {
        std::uint32_t entryFormId =
            static_cast<std::uint32_t>(formIdValue.GetNumber());
        if (entryFormId == spellId) {
          ...
          logger::info("MagicMenuUIHook: Updated spell {:08X} '{}' -> '{}'",
                       spellId, originalName, modifiedName);
          return true;
        }
      }
      ...
      if (entry.GetMember("text", &textValue) && textValue.IsString()) {
        ...
        if (entryText && strcmp(entryText, originalName) == 0) {
          ...
          logger::info(
              "MagicMenuUIHook: Updated spell by name match '{}' -> '{}'",
  ...
  static void TryUpdateDescription(RE::GFxValue &entry, RE::FormID spellId) {
    ...
    const char *descFields[] = {"description", "desc", "effectDescription",
                                "info"};
    for (const char *fieldName : descFields) {
      ...
      if (entry.GetMember(fieldName, &descValue)) {
        ...
        logger::info(
            "MagicMenuUIHook: Updated description field '{}' for spell {:08X}",
            fieldName, spellId);
        return;
      }
    }
  ...
  static void LogGfxStructure(RE::GFxValue &obj, const std::string &path,
                              int depth) {
    if (depth > 3)
      return; // Limit depth to avoid spam
    ...
    const char *members[] = {"Menu_mc",        "itemList",  "entryList",
                             "InventoryLists", "spellList", "text",
                             "formId"};
    for (const char *member : members) {
      ...
      if (obj.GetMember(member, &child)) {
        ...
        if (child.IsObject()) {
          ...
          logger::info("MagicMenuUIHook: Found {} (array, size={})", childPath,
                       child.GetArraySize());
        } else if (child.IsString()) {
          logger::info("MagicMenuUIHook: Found {} = '{}'", childPath,
                       child.GetString());
  ...
  static void Install() {
    ...
    // This is called after the menu is rendered, so we can safely modify GFx
    // values
    REL::Relocation<std::uintptr_t> vtbl{RE::VTABLE_MagicMenu[0]};
    func = vtbl.write_vfunc(0x6, thunk);

    logger::info("SpellEffectivenessHook: MagicMenu::PostDisplay hook "
                 "installed (UI-level, safe)");
  }
};
...
void SpellEffectivenessHook::InstallDisplayHooks() {
  ...
  // We now directly modify SpellItem::fullName when tracking early-learned
  // spells. This works with vanilla UI, SkyUI, and any other UI mod without
  // needing hooks.
  //
  ...
  logger::info("SpellEffectivenessHook: Using direct spell name modification "
               "(no UI hooks needed)");
}
...
void SpellEffectivenessHook::ApplyEffectivenessScalingFast(
  ...
  if (baseEffect) {
    ...
    if (isBinaryEffect) {
      float progressPercent = ProgressionManager::GetSingleton()
                                  ->GetProgress(spellId)
                                  .progressPercent *
                              100.0f;
      if (progressPercent < m_settings.binaryEffectThreshold) {
        a_effect->magnitude = 0.0f;
        logger::trace("SpellEffectivenessHook: Binary effect {:08X} blocked",
                      spellId);
        return;
      }
    }
  ...
  a_effect->duration *= effectiveness;

  // Use trace level logging to reduce overhead (only visible with verbose
  // logging)
  logger::trace("SpellEffectivenessHook: Scaled {:08X} to {}%", spellId,
                static_cast<int>(effectiveness * 100));
}

// Legacy version for compatibility (calls fast version after player check)
void SpellEffectivenessHook::ApplyEffectivenessScaling(
    RE::ActiveEffect *a_effect) {
  if (!a_effect) {
    return;
  }
...
void SpellEffectivenessHook::SetSettings(
    const EarlyLearningSettings &settings) {
  std::unique_lock<std::shared_mutex> lock(m_mutex);
  m_settings = settings;
  logger::info("SpellEffectivenessHook: Settings updated - enabled: {}, "
               "unlock: {}%, min: {}%, max: {}%",
               settings.enabled, settings.unlockThreshold,
               settings.minEffectiveness, settings.maxEffectiveness);
}
...
void SpellEffectivenessHook::AddEarlyLearnedSpell(RE::FormID formId) {
  ...
  logger::info(
      "SpellEffectivenessHook: Added spell {:08X} to early-learned set",
      formId);
}
...
void SpellEffectivenessHook::RemoveEarlyLearnedSpell(RE::FormID formId) {
  ...
  logger::info("SpellEffectivenessHook: Removed spell {:08X} from "
               "early-learned set (mastered)",
               formId);
}
...
void SpellEffectivenessHook::SetPowerSteps(
    const std::vector<PowerStep> &steps) {
  std::unique_lock<std::shared_mutex> lock(m_mutex);
  ...
  logger::info("SpellEffectivenessHook: Updated power steps ({} steps)",
               m_powerSteps.size());
  for (size_t i = 0; i < m_powerSteps.size(); ++i) {
    logger::info("  Step {}: {}% XP -> {}% power ({})", i + 1,
...
float SpellEffectivenessHook::GetSteppedEffectiveness(
    RE::FormID spellFormId) const {
...
float SpellEffectivenessHook::CalculateEffectiveness(
    RE::FormID spellFormId) const {
...
void SpellEffectivenessHook::GrantEarlySpell(RE::SpellItem *spell) {
  ...
  // Track as early-learned even if player already has it (but not if already
  // tracked)
  if (!alreadyTracked) {
    hook->AddEarlyLearnedSpell(formId);
    hook->UpdateSpellDisplayCache(formId, spell);

    // Apply the modified name and description to the actual spell (works with
    // SkyUI)
    hook->ApplyModifiedSpellName(formId);
    hook->ApplyModifiedDescriptions(formId);

    logger::info("SpellEffectivenessHook: Now tracking {} ({:08X}) as "
                 "early-learned (had spell: {})",
                 spell->GetName(), formId, alreadyHasSpell);

    // Notify UI
    UIManager::GetSingleton()->UpdateSpellState(std::format("0x{:08X}", formId),
                                                "weakened");
  } else {
    logger::trace("SpellEffectivenessHook: Spell {} ({:08X}) already tracked "
                  "as early-learned",
                  spell->GetName(), formId);
  }
}
...
void SpellEffectivenessHook::MarkMastered(RE::FormID spellFormId) {
  ...
  logger::info("SpellEffectivenessHook: Spell {:08X} mastered - nerf removed, "
               "name restored",
               spellFormId);
...
void SpellEffectivenessHook::RemoveEarlySpellFromPlayer(
  ...
  if (!spell) {
    logger::warn(
        "SpellEffectivenessHook: Could not find spell {:08X} to remove",
        spellFormId);
    return;
  }
  ...
  if (player->HasSpell(spell)) {
    ...
    logger::info("SpellEffectivenessHook: Removed early-learned spell {} "
                 "({:08X}) from player",
...
void SpellEffectivenessHook::CheckAndRegrantSpell(RE::FormID spellFormId) {
  ...
  logger::info("SpellEffectivenessHook: CheckAndRegrantSpell {:08X} - progress "
               "{:.1f}%, threshold {:.1f}%",
               spellFormId, progressPercent, m_settings.unlockThreshold);

  // Check if progress is above early learning threshold but below 100%
  // (mastery)
  if (progressPercent >= m_settings.unlockThreshold &&
      progressPercent < 100.0f) {
    // Get the spell
    auto *spell = RE::TESForm::LookupByID<RE::SpellItem>(spellFormId);
    if (!spell) {
      logger::warn(
          "SpellEffectivenessHook: Could not find spell {:08X} to regrant",
          spellFormId);
      return;
    }
    ...
    if (!isTracked) {
      ...
      logger::info("SpellEffectivenessHook: Tracking spell {} ({:08X}) as "
                   "early-learned - progress {:.1f}%",
...
void SpellEffectivenessHook::UpdateSpellDisplayCache(RE::FormID spellFormId,
                                                     RE::SpellItem *spell) {
  if (!spell) {
  ...
  // This prevents stacking modifications like "Spell (Learning - 20%) (Learning
  // - 35%)"
  std::string originalName;
  {
    ...
    if (it != m_originalSpellNames.end()) {
      ...
      logger::info(
          "SpellEffectivenessHook: Stored original name for {:08X}: '{}'",
  ...
  if (step < numSteps - 1) { // Not mastered (last step is always 100%)
    modifiedName =
        std::format("{} (Learning - {}%)", originalName, powerPercent);
  } else {
  ...
  logger::info("SpellEffectivenessHook: Updated display cache for {:08X} - {} "
               "(step {}: {}%)",
...
void SpellEffectivenessHook::RefreshAllSpellDisplays() {
  logger::info(
      "SpellEffectivenessHook: Refreshing all spell displays after load...");
  ...
  for (RE::FormID spellId : spellsCopy) {
    ...
    if (spell) {
      ...
      if (player && !player->HasSpell(spell)) {
        // Re-grant it
        player->AddSpell(spell);
        logger::info("SpellEffectivenessHook: Re-granted spell {} on load",
                     spell->GetName());
      }
    }
  }

  logger::info("SpellEffectivenessHook: Refreshed {} spell displays",
               spellsCopy.size());
}
...
void SpellEffectivenessHook::ApplyModifiedSpellName(RE::FormID spellFormId) {
  ...
  logger::info(
      "SpellEffectivenessHook: Applied modified name to spell {:08X}: '{}'",
...
void SpellEffectivenessHook::RestoreOriginalSpellName(RE::FormID spellFormId) {
  ...
  logger::info(
      "SpellEffectivenessHook: Restored original name for spell {:08X}: '{}'",
      spellFormId, originalName);
}

void SpellEffectivenessHook::RefreshAllSpellNames() {
  logger::info(
      "SpellEffectivenessHook: Refreshing all spell names and descriptions...");
  ...
  logger::info("SpellEffectivenessHook: Refreshed {} spell names/descriptions",
               spellsCopy.size());
}
...
void SpellEffectivenessHook::ApplyModifiedDescriptions(RE::FormID spellFormId) {
  ...
  for (auto *effect : spell->effects) {
    ...
    {
      std::unique_lock<std::shared_mutex> lock(m_mutex);
      if (m_originalEffectDescriptions.find(effectId) ==
          m_originalEffectDescriptions.end()) {
        if (baseEffect->magicItemDescription.data()) {
          m_originalEffectDescriptions[effectId] =
              baseEffect->magicItemDescription.data();
          logger::info("SpellEffectivenessHook: Stored original description "
                       "for effect {:08X}: '{}'",
    ...
    // Create modified description by replacing <mag> and <dur> with scaled
    // values
    ...
    modifiedDesc =
        "[" + std::to_string(powerPercent) + "% Power] " + modifiedDesc;

    // Apply the modified description
    baseEffect->magicItemDescription = modifiedDesc;

    logger::info(
        "SpellEffectivenessHook: Modified description for effect {:08X}: '{}'",
        effectId, modifiedDesc);
  }
}

void SpellEffectivenessHook::RestoreOriginalDescriptions(
    RE::FormID spellFormId) {
  auto *spell = RE::TESForm::LookupByID<RE::SpellItem>(spellFormId);
  if (!spell) {
    return;
  ...
  for (auto *effect : spell->effects) {
    ...
    if (shouldRestore && !originalDesc.empty()) {
      ...
      logger::info("SpellEffectivenessHook: Restored original description for "
                   "effect {:08X}: '{}'",
...
void SpellEffectivenessHook::RefreshAllDescriptions() {
  ...
  logger::info("SpellEffectivenessHook: Refreshed {} spell descriptions",
               spellsCopy.size());
}
...
float SpellEffectivenessHook::GetScaledMagnitude(
    RE::SpellItem *spell, float originalMagnitude) const {
...
static std::string ScaleDescriptionNumbers(const std::string &description,
  ...
  // - Followed by word boundary, space, or common suffixes like "points",
  // "damage", "%"
...
std::string
SpellEffectivenessHook::GetScaledSpellDescription(RE::SpellItem *spell) const {
  if (!spell) {
    return "";
  }
  ...
  for (size_t i = 0; i < spell->effects.size(); ++i) {
    ...
    if (!effectDesc.empty()) {
      ...
      while (magPos != std::string::npos) {
        effectDesc.replace(magPos, 5,
                           std::to_string(static_cast<int>(displayMag)));
  ...
  if (isWeakened && !result.empty()) {
    result = "[" + std::to_string(static_cast<int>(effectiveness * 100)) +
             "% Power] " + result;
  }
...
void SpellEffectivenessHook::OnGameSaved(
    SKSE::SerializationInterface *a_intfc) {
  std::shared_lock<std::shared_mutex> lock(m_mutex);

  if (!a_intfc->OpenRecord(kEarlyLearnedRecord, 1)) {
    logger::error("SpellEffectivenessHook: Failed to open early-learned record "
                  "for saving");
    return;
  }
...
void SpellEffectivenessHook::OnGameLoaded(
    SKSE::SerializationInterface *a_intfc) {
  {
    std::unique_lock<std::shared_mutex> lock(m_mutex);
    ...
    while (a_intfc->GetNextRecordInfo(type, version, length)) {
      if (type == kEarlyLearnedRecord) {
        ...
        if (!a_intfc->ReadRecordData(&count, sizeof(count))) {
          logger::error(
              "SpellEffectivenessHook: Failed to read early-learned count");
          return;
        }
        ...
        for (uint32_t i = 0; i < count; ++i) {
          ...
          if (!a_intfc->ReadRecordData(&formId, sizeof(formId))) {
            logger::error(
                "SpellEffectivenessHook: Failed to read formId at index {}", i);
            break;
          }
          ...
          if (a_intfc->ResolveFormID(formId, resolvedId)) {
            ...
            logger::warn(
                "SpellEffectivenessHook: Failed to resolve formId {:08X}",
                formId);
          }
        }

        logger::info("SpellEffectivenessHook: Loaded {} early-learned spells",
                     m_earlyLearnedSpells.size());
      }
    }
  }
  ...
  SKSE::GetTaskInterface()->AddTask([this]() { RefreshAllSpellDisplays(); });
}
...
void SpellEffectivenessHook::OnRevert(SKSE::SerializationInterface *a_intfc) {
  ...
  logger::info("SpellEffectivenessHook: Cleared early-learned spells and "
               "display cache on revert");
}
