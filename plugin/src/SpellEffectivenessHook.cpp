...
struct AdjustForPerksHook
{
    ...
    static void Install()
    {
        ...
        logger::info("SpellEffectivenessHook: AdjustForPerks hook installed at vtable index 0");
    }
};
...
void SpellEffectivenessHook::Install()
{
    ...
    // Hook for magnitude scaling (effectiveness) - scales early-learned spell damage
...
void SpellEffectivenessHook::ApplyEffectivenessScalingFast(RE::ActiveEffect* a_effect)
{
    ...
    logger::trace("SpellEffectivenessHook: Scaled {:08X} to {}%", spellId, static_cast<int>(effectiveness * 100));
}
...
void SpellEffectivenessHook::GrantEarlySpell(RE::SpellItem* spell)
{
    ...
    if (!alreadyHasSpell) {
        ...
        snprintf(notification, sizeof(notification), "%s %s learned (weakened)", 
            label, spell->GetName());
        RE::DebugNotification(notification);
    }
