...
void SpellTomeHook::OnSpellTomeRead(RE::TESObjectBOOK *a_book,
  ...
  if (player->HasSpell(a_spell)) {
    ...
    logger::info("SpellTomeHook: Player already knows '{}', keeping tome",
                 a_spell->GetName());
    return;
  }
  ...
  if (!hook->m_settings.enabled || !hook->m_settings.useProgressionSystem) {
    logger::info(
        "SpellTomeHook: Using VANILLA mode - teaching spell instantly");
    ...
    RE::TESObjectREFR *removeFrom =
        container ? container : player->AsReference();
    if (removeFrom) {
      removeFrom->RemoveItem(a_book, 1, RE::ITEM_REMOVE_REASON::kRemove,
                             nullptr, nullptr);
    }
    ...
    logger::info("SpellTomeHook: Vanilla mode - taught '{}', consumed tome",
                 a_spell->GetName());
    return;
  }
  ...
  // =========================================================================
  logger::info("SpellTomeHook: Using PROGRESSION mode for spell '{}'",
               a_spell->GetName());
  ...
  // =========================================================================
  logger::info("SpellTomeHook: Checking prerequisites for spell {:08X} '{}' - "
               "requirePrereqs={}",
               spellFormId, a_spell->GetName(),
               hook->m_settings.requirePrereqs);
  ...
  if (hook->m_settings.requirePrereqs) {
    ...
    logger::info(
        "SpellTomeHook: Prereqs for {:08X}: {} hard, {} soft (need {})",
        spellFormId, reqs.hardPrereqs.size(), reqs.softPrereqs.size(),
        reqs.softNeeded);
    ...
    if (hasAnyPrereqs) {
      ...
      for (RE::FormID prereqId : reqs.hardPrereqs) {
        ...
        logger::info("SpellTomeHook:   - HARD {:08X} '{}' mastered={}",
                     prereqId, prereqSpell ? prereqSpell->GetName() : "UNKNOWN",
                     mastered);
        if (!mastered) {
      ...
      logger::info("SpellTomeHook: hardMet={}, softMet={} ({}/{})", hardMet,
                   softMet, softMastered, softNeeded);
      ...
      if (!hardMet || !softMet) {
        ...
        if (hook->m_settings.showNotifications) {
          ...
          if (!hardMet) {
            ...
            for (RE::FormID prereqId : unmetHard) {
              auto *prereqSpell =
                  RE::TESForm::LookupByID<RE::SpellItem>(prereqId);
              if (prereqSpell) {
            ...
            for (size_t i = 0; i < spellNames.size(); i++) {
              if (i == 0)
                msg += spellNames[i];
              else if (i == spellNames.size() - 1)
                msg += " and " + spellNames[i];
              else
                msg += ", " + spellNames[i];
            }
          } else if (!softMet) {
            ...
            msg = "You need to master " + std::to_string(stillNeeded) +
                  " more related spell";
            if (stillNeeded > 1)
              msg += "s";
          }
        ...
        logger::info("SpellTomeHook: Player missing prerequisites for '{}' "
                     "(hardMet={}, softMet={})",
  ...
  if (hook->m_settings.requireSkillLevel) {
    ...
    if (spellEffect && spellEffect->baseEffect) {
      int minimumSkill =
          static_cast<int>(spellEffect->baseEffect->data.minimumSkill);
      ...
      if (minimumSkill > 0) {
        ...
        if (playerSkill < minimumSkill) {
          ...
          logger::info("SpellTomeHook: Player lacks skill for '{}' (needs {}, "
                       "has {:.0f})",
                       a_spell->GetName(), minimumSkill,
                       player->AsActorValueOwner()->GetActorValue(school));
          return; // Don't learn, keep the tome
        }
      }
    }
  ...
  // This is allowed even if XP was already granted (player might have changed
  // target)
  ...
  if (hook->m_settings.grantXPOnRead && !alreadyGrantedXP) {
    ...
    logger::info("SpellTomeHook: Granted {:.1f} XP ({:.0f}% of {:.1f} "
                 "required) for '{}'",
                 xpToGrant, hook->m_settings.xpPercentToGrant, requiredXP,
                 a_spell->GetName());

    // Show notification
    if (hook->m_settings.showNotifications) {
      char msg[256];
      snprintf(msg, sizeof(msg), "You begin to study %s...",
               a_spell->GetName());
      RE::SendHUDMessage::ShowHUDMessage(msg);
    }
  } else if (alreadyGrantedXP) {
    logger::info(
        "SpellTomeHook: XP already granted for '{}' - no additional XP",
        a_spell->GetName());

    // Show different notification
    if (hook->m_settings.showNotifications) {
      char msg[256];
      snprintf(msg, sizeof(msg), "You review %s... (no additional insight)",
               a_spell->GetName());
      RE::SendHUDMessage::ShowHUDMessage(msg);
    }
  }
...
bool SpellTomeHook::Install() {
  ...
  if (!pattern.match(hookAddr)) {
    logger::error("SpellTomeHook: Pattern verification failed at {:X} - game "
                  "version mismatch?",
                  hookAddr);
    return false;
  }
  ...
  std::uintptr_t callbackAddr =
      reinterpret_cast<std::uintptr_t>(&OnSpellTomeRead);
  std::uintptr_t returnAddr =
      hookAddr + JumpOffset; // Where to jump after our code
  Patch patch(callbackAddr, returnAddr);
  patch.ready();

  // Verify patch size
  if (patch.getSize() > PatchSize) {
    logger::error("SpellTomeHook: Patch too large ({} bytes, max {})",
                  patch.getSize(), PatchSize);
    return false;
  }

  logger::info("SpellTomeHook: Patch size: {} bytes (max {})", patch.getSize(),
               PatchSize);
...
float SpellTomeHook::GetXPMultiplier(RE::FormID spellFormId) const {
  ...
  if (m_settings.tomeInventoryBoost && PlayerHasSpellTome(spellFormId)) {
    ...
    logger::trace("SpellTomeHook: Tome inventory boost active for {:08X}, "
                  "multiplier = {:.2f}",
...
void SpellTomeHook::MarkTomeXPGranted(RE::FormID spellFormId) {
  ...
  logger::info("SpellTomeHook: Marked spell {:08X} as having received tome XP",
               spellFormId);
}
