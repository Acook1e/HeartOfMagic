...
void UIManager::ShowPanel()
{
  ...
  if (menuCursor) {
    ...
    logger::info("UIManager: MenuCursor state - showCursorCount={}, pos=({}, {})", menuCursor->showCursorCount,
  ...
  logger::info("UIManager: ShowPanel - after Show: order={}, hidden={}", m_prismaUI->GetOrder(m_view),
               m_prismaUI->IsHidden(m_view));
  ...
  SKSE::GetTaskInterface()->AddTask([this, pauseGame]() {
    ...
    auto* menuCursor = RE::MenuCursor::GetSingleton();
    if (menuCursor) {
      logger::info("UIManager: Deferred Focus applied (pauseGame={}, hasFocus={}, cursorCount={})", pauseGame,
                   hasFocusNow, menuCursor->showCursorCount);
    } else {
...
void UIManager::ScheduleFocusRetry(bool pauseGame, int attempt)
{
  ...
  SKSE::GetTaskInterface()->AddTask([this, pauseGame, attempt]() {
    ...
    if (hasFocus && cursorVisible) {
      logger::info("UIManager: Focus verified on attempt {} (cursorCount={})", attempt,
                   menuCursor ? menuCursor->showCursorCount : -1);
      return;  // Success!
    }

    logger::info("UIManager: Focus retry attempt {} (hasFocus={}, cursorVisible={})", attempt, hasFocus, cursorVisible);
...
void UIManager::OnScanSpells(const char* argument)
{
  ...
  if (useTomeMode) {
    ...
    scanConfig.fields.castingType = true;
    scanConfig.fields.plugin      = true;
...
void UIManager::OnLoadSpellTree(const char* argument)
{
  ...
  try {
    ...
    if (file.is_open()) {
      ...
      logger::info("UIManager: Tree validation - {}/{} valid, {} resolved, {} invalid", validationResult.validNodes,
                   validationResult.totalNodes, validationResult.resolvedFromPersistent, validationResult.invalidNodes);
...
void UIManager::OnGetPlayerKnownSpells(const char* argument)
{
  ...
  auto isValidCombatSpell = [](RE::SpellItem* spell) -> bool {
    ...
    if (school != RE::ActorValue::kAlteration && school != RE::ActorValue::kConjuration &&
        school != RE::ActorValue::kDestruction && school != RE::ActorValue::kIllusion &&
      ...
      for (uint32_t i = 0; i < spellList->numSpells; ++i) {
        ...
        if (spell && foundSpells.find(spell->GetFormID()) == foundSpells.end()) {
          if (isValidCombatSpell(spell)) {
            ...
            logger::trace("UIManager: Skipping non-combat spell/ability: {} ({:08X})", spell->GetName(),
                          spell->GetFormID());
          }
        }
      }
...
void UIManager::OnSetTreePrerequisites(const char* argument)
{
  ...
  try {
    ...
    for (const auto& entry : request) {
        ...
        for (const auto& prereqStr : entry["hardPrereqs"]) {
          if (prereqStr.is_string()) {
            try {
              ...
              logger::warn("UIManager: Could not parse hardPrereq '{}' for spell {:08X}", prereqStr.get<std::string>(),
                           formId);
            }
          }
        }
        ...
        for (const auto& prereqStr : entry["softPrereqs"]) {
          if (prereqStr.is_string()) {
            try {
              ...
              logger::warn("UIManager: Could not parse softPrereq '{}' for spell {:08X}", prereqStr.get<std::string>(),
                           formId);
            }
          }
        }
      ...
      if (reqs.hardPrereqs.empty() && reqs.softPrereqs.empty() && entry.contains("prereqs") &&
          entry["prereqs"].is_array()) {
      ...
      if (!reqs.hardPrereqs.empty() || !reqs.softPrereqs.empty()) {
        auto* spell = RE::TESForm::LookupByID<RE::SpellItem>(formId);
        logger::info("UIManager: Setting prereqs for {:08X} '{}': {} hard, {} soft (need {})", formId,
                     spell ? spell->GetName() : "UNKNOWN", reqs.hardPrereqs.size(), reqs.softPrereqs.size(),
                     reqs.softNeeded);
      }
  ...
  return json{{"hotkey", "F8"},
              ...
              {"earlySpellLearning",
               {{"enabled", true},
                ...
                {"powerSteps", json::array({{{"xp", 25}, {"power", 20}, {"label", "Budding"}},
              ...
              {"notifications", {{"weakenedSpellNotifications", true}, {"weakenedSpellInterval", 10.0f}}},
...
void UIManager::OnLoadUnifiedConfig(const char* argument)
{
  ...
  if (std::filesystem::exists(legacyLLMPath)) {
    try {
      ...
      json llmConfig = {{"apiKey", SafeJsonValue<std::string>(legacyLLM, "apiKey", "")},
  ...
  if (unifiedConfig.contains("spellTomeLearning") && !unifiedConfig["spellTomeLearning"].is_null()) {
    ...
    logger::info("UIManager: Applied SpellTomeHook settings - useProgressionSystem: {}, requirePrereqs: {}, "
                 "requireAllPrereqs: {}, requireSkillLevel: {}",
                 tomeSettings.useProgressionSystem, tomeSettings.requirePrereqs, tomeSettings.requireAllPrereqs,
                 tomeSettings.requireSkillLevel);
  }
...
void UIManager::OnSaveUnifiedConfig(const char* argument)
{
  ...
  try {
    ...
    if (newConfig.contains("notifications") && !newConfig["notifications"].is_null()) {
      ...
      auto* castHandler = SpellCastHandler::GetSingleton();
      castHandler->SetWeakenedNotificationsEnabled(
          SafeJsonValue<bool>(notifConfig, "weakenedSpellNotifications", true));
      castHandler->SetNotificationInterval(SafeJsonValue<float>(notifConfig, "weakenedSpellInterval", 10.0f));
...
void UIManager::NotifyProgressUpdate(RE::FormID formId, float currentXP, float requiredXP)
{
  ...
  logger::trace("UIManager: Sending progress update to UI - formId: {}, XP: {:.1f}/{:.1f}, unlocked: {}", ss.str(),
                currentXP, requiredXP, progress.unlocked);
...
void UIManager::OnCheckLLM(const char* argument)
{
  ...
  if (!hasApiKey) {
    logger::warn(
        "UIManager: OpenRouter API key not configured. Edit: Data/SKSE/Plugins/SpellLearning/openrouter_config.json");
  } else {
...
void UIManager::OnLLMGenerate(const char* argument)
{
  ...
  try {
    ...
    logger::info("UIManager: LLM generate request for school: {}, spellData length: {}, model: {}, maxTokens: {}, "
                 "multiPrereqs: {}, aggressiveValidation: {}",
                 schoolName, spellData.length(), config.model, config.maxTokens, allowMultiplePrereqs,
    ...
    std::string systemPrompt =
        R"(You are a Skyrim spell tree architect. Your task is to create a logical spell learning tree for a single magic school. You MUST return ONLY valid JSON - no explanations, no markdown code blocks, just raw JSON.
...
- Focus on thematic connections over strict tier progression)";
    ...
    OpenRouterAPI::SendPromptAsync(
        effectiveSystemPrompt, userPrompt, [instance, schoolName](const OpenRouterAPI::Response& response) {
          json result;
          ...
          if (response.success) {
            ...
            logger::info("UIManager: OpenRouter success for {}, response length: {}", schoolName,
    ...
    try {
      ...
      if (!content.empty()) {
        ...
        if (delimPos != std::string::npos) {
          ...
          logger::info("UIManager: Found LLM response, success={}, length={}", success, response.length());
          ...
          logger::warn("UIManager: Response missing delimiter, content: {}", content.substr(0, 50));
        }
      }
    } catch (const std::exception& e) {
...
void UIManager::OnProceduralPythonGenerate(const char* argument)
{
  ...
  auto* instance = GetSingleton();
  if (!instance || !instance->m_prismaUI)
    return;
